...
export class MusicPlayerService implements OnDestroy {
  ...
  private playStream(url: string): void {
    if (!this.audioElement) {
      ...
      // Sincroniza o estado quando o stream falha (ex: erro de rede, CORS, CSP)
      this.audioElement.addEventListener('error', () => {
        this._state.update(s => ({ ...s, isPlaying: false }));
      });
    }

    // Recarrega se a URL mudou ou se o elemento está em estado de erro
    const urlChanged  = this.audioElement.src !== url;
    const hasError    = this.audioElement.error != null;
    ...
    this.audioElement.play().catch(() => {
      // Garante que o estado reflita a realidade quando play() é rejeitado
      this._state.update(s => ({ ...s, isPlaying: false }));
    });
  }
