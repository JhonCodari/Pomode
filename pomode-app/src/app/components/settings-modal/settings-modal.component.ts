...
export class SettingsModalComponent {
  ...
  saveSettings(): void {
    ...
    this.analyticsService.trackSettingsSaved({
  ...
  resetToDefault(): void {
    ...
    this.analyticsService.trackSettingsResetToDefault();
  ...
  async requestNotificationPermission(): Promise<void> {
    if (!('Notification' in window)) {
      this.analyticsService.trackNotificationPermissionRequested('unsupported');
      this.toastService.warning(this.translate.instant('SETTINGS.NOTIFICATIONS.NOT_SUPPORTED'));
      return;
    }

    const permission = await Notification.requestPermission();
    this.analyticsService.trackNotificationPermissionRequested(
